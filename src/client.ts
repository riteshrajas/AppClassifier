...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['APPCLASSIFIER_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['APPCLASSIFIER_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Appclassifier API.
 */
export class Appclassifier {
   ...
   * @param {string} [opts.baseURL=process.env['APPCLASSIFIER_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('APPCLASSIFIER_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.AppclassifierError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the Appclassifier client with an apiKey option, like new Appclassifier({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Appclassifier.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('APPCLASSIFIER_LOG'), "process.env['APPCLASSIFIER_LOG']", this) ??
      defaultLogLevel;
  ...
  static Appclassifier = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static AppclassifierError = Errors.AppclassifierError;
...
Appclassifier.PetResource = PetResource;
Appclassifier.Store = Store;
Appclassifier.UserResource = UserResource;
export declare namespace Appclassifier {
